...
public class NewsFeedProvider extends AbstractEntityListProvider {
    ...
    public Request getEntities(Request request) {
        ...
        // Fetch NASA RSS Feed
        if (isRunningAsJavascriptDemoInCodenameOneInitializr()) {
            try (InputStream input = CN.getResourceAsStream("/demofeed.xml")) {
                String contents = Util.readToString(input);
                parseXMLData(request, contents.getBytes("UTF-8"));
        ...
        Rest.get("https://www.nasa.gov/rss/dyn/breaking_news.rss")
                ...
                .fetchAsBytes(v -> {
                    parseXMLData(request, v.getResponseData());
                });
        return request;
    }

    private void parseXMLData(Request request, byte[] data) {
        ...
        parser.parseXML(new InputStreamReader(new ByteArrayInputStream(data)))
                .onResult((res, err) -> {
                    if (err != null) {
    ...
    private boolean isRunningAsJavascriptDemoInCodenameOneInitializr() {
        return CN.getProperty("browser.window.location.href", "").indexOf("start.codenameone.com") >= 0;
    }
}
